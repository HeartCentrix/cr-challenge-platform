...
describe('AdminStats', () => {
  ...
  it('cancels stale lists, uses cursor batches and resets on bucket changes without refreshing the chart', () => {
    ...
    second.flush({ items: [{ ...row, id: 101, aiMarkerDetected: false, performance: { ...row.performance, totalScore: 80 } }], total: 2, nextCursor: null });
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelector('.candidate-region').textContent).toContain('Texas');
    const flags = fixture.nativeElement.querySelectorAll('.ai-used');
    expect(flags.length).toBe(2);
    expect(flags[0].textContent).toBe('Yes');
    expect(flags[1].textContent).toBe('No');
    expect(flags[0].title).toContain('not proof of cheating');
    expect(fixture.nativeElement.querySelector('th[aria-sort="descending"]').textContent).toBe('Total score');
    expect(Array.from(fixture.nativeElement.querySelectorAll('th'), (th: any) => th.textContent)).not.toContain('Test cases');
    expect(fixture.componentInstance.rows().map(r => r.id)).toEqual([100, 101]);
    fixture.componentInstance.loadMore();
    http.expectNone(r => r.url.endsWith('/candidates'));
    fixture.destroy();
  });
