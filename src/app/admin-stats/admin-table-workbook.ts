...
export function tableWorkbook(rows: CandidateRow[], filters: Record<string, string | number>, time = new AdminTime(), exportedAt = new Date()) {
  ...
  const sheet = reportSheet(book, stamp, 'Candidates', ['Candidate', 'Candidate ID', 'Email', 'Phone', 'Campaign', 'Challenges',
    'Submissions', 'Test cases passed', 'Test cases total', 'Pass rate', 'Total score', 'Average score', 'Total time', 'Last submission (local)', 'UTC offset'],
    [30, 16, 36, 22, 26, 16, 16, 18, 18, 16, 16, 18, 18, 25, 18],
  ...
  for (const item of rows) {
    ...
    const r = sheet.addRow([item.fullName || 'Unnamed candidate', item.id, item.email, item.phone, item.sourceCampaign || 'Not recorded',
      ...
      time.excelDate(p.lastSubmittedAt), time.parts(p.lastSubmittedAt)?.offset]);
  ...
  for (const [key, label] of Object.entries({ search: 'Search', campaign: 'Campaign', startDate: 'Start date', endDate: 'End date',
